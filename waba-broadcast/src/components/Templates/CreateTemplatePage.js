...
import { checkTemplateNameExists } from '../../API/TemplateList/CheckTemplateName';
...
const CreateTemplatePage = () => {
    ...
    const [isCheckingTemplateName, setIsCheckingTemplateName] = useState(false);
    ...
    // Debounced template name uniqueness check
    useEffect(() => {
        if (isEditMode) return;
        const name = templateDetails.templateName.trim();
        if (!name) {
            setIsCheckingTemplateName(false);
            return;
        }

        const timer = setTimeout(async () => {
            setIsCheckingTemplateName(true);
            try {
                const userTokenRaw = sessionStorage.getItem('userToken');
                const userToken = userTokenRaw ? JSON.parse(userTokenRaw) : {};
                const result = await checkTemplateNameExists(userToken?.userId || userToken?.username || '');
                if (result.success && result.data.length > 0) {
                    const normalizedInput = normalizeTemplateName(name).toLowerCase();
                    const exists = result.data.some((t) => {
                        const existingName = normalizeTemplateName(t.TemplateName || '').toLowerCase();
                        return existingName === normalizedInput;
                    });
                    if (exists) {
                        setTemplateNameError('Template name already exists. Please use a different name.');
                    }
                }
            } catch (e) {
    ...
    }, [templateDetails.templateName, isEditMode]);
            ...
            <Box className={`${step === 2 ? styles.builderWrapper : styles.detailsWrapper}`}>
                ...
                {step === 1 && (
                    <TemplateDetailsStepSection
                        ...
                        isCheckingTemplateName={isCheckingTemplateName}
