...
export const useConversation = (selectedCustomer, onConversationRead, onViewConversationRead) => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (files.length > MAX_FILES_COUNT) {
            toast.error(`You can only upload up to ${MAX_FILES_COUNT} files at once.`);
            return;
        }

        // Validate file types
        const disallowed = files.filter((f) => !isFileTypeAllowed(f));
        if (disallowed.length > 0) {
            toast.error(`Some files have unsupported formats: ${disallowed.map((f) => f.name).join(', ')}`);
            return;
        }
        ...
        if (oversized.length > 0) {
            toast.error(`Some files exceed the 16 MB limit: ${oversized.map((f) => f.name).join(', ')}`);
            return;
        }
